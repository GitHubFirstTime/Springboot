...
package com.rlc.midServer.modules.stereoLayer.common.interceptor;
...
import com.rlc.midServer.common.config.HttpHelper;
import com.rlc.midServer.common.security.Digests;
import com.rlc.midServer.common.security.TokenUtil;
import com.rlc.midServer.common.service.BaseService;
...
public class TokenInterceptor extends BaseService implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if(java.util.Objects.equals("POST", request.getMethod())) {//post请求
			logger.info("进入令牌拦截器，请求方法为：{}，请求方式POST",methodName);
	        ...
	        if(StringUtils.isNotBlank(authorizationStr)) {
	        	logger.info("authorizationStr___"+authorizationStr);
				String signContent = null;//签名内容
				try {
					signContent = authorizationStr.split(" ")[1];
				} catch (Exception e) {
					logger.error("authorizationStr",e);
					if (e instanceof ArrayIndexOutOfBoundsException){
						Map<String, Object> resultMap0 = Maps.newHashMap();
						StereoResult stereoResult = StereoResult.ERROR().MESSAGE("無签名信息").CODE("400");
				...
				try {
					...
					logger.info(methodName+"方法的jwt:{}",jwt);
				} catch (Exception e) {
					logger.error("jwt 认证失败,缺少令牌");
	        	...
	        	if(jwtBool) {//令牌通过
	        		if(StringUtils.isNotBlank(signContent)) {
	        			if(StringUtils.isNotBlank(body)&&!"null".equals(body)&&!"NULL".equals(body)) {
	        		    	...
	        		    	logger.info("TokenInterceptor_body不为空时："+replaceEnter(bodyJSONObject.toString()));
	        		    	String sign = Digests.encoderByMd5(appId+"\n"+replaceEnter(bodyJSONObject.toString())+"\n"+"fmb-api");
	        		    	if(Objects.equal(sign, signContent)) {
	        		    		bool = true;
	        		    		logger.info("签名验证通过");
	        		    	}else {
	        		    		logger.error("TokenInterceptor签名错误，接收：{},生成：{}",signContent,sign);
...
//	        				JSONObject bodyJSONObject = JSONObject.fromObject("{}");
	        		    	...
	        		    	if(Objects.equal(sign, signContent)) {
	        		    		...
	        		    		logger.error("TokenInterceptor body is blank 时 签名错误，接收：{},生成：{}",signContent,sign);
	        		    		resultMap.put("code", 400);
	    	        			resultMap.put("msg", "");
	    	        			resultMap.put("desc", "签名错误");
	    	        			resultMap.put("data", new JSONObject());
...
//		    		Map<String, Object> resultMap = Maps.newHashMap();
			...
			logger.info("进入tokenintercept--get");
			logger.info("进入令牌拦截器，请求方法为：{}，请求方式GET",methodName);
			String token = request.getParameter("token");
			String appId = request.getParameter("appId");
			String authorizationStr = request.getHeader("Authorization");
			logger.info("GET请求方式之签名信息{}",authorizationStr);
				...
				if((boolean) resultMap.get("success")) {
					if(resultMap.containsKey("markData")) {
						...
						if(Objects.equal(markData, objectId)) {
							...
							bool = true;
							logger.info("validateToken:令牌验证通过");
						}else {
							bool = false;
							resultMap.put("success", false);
							resultMap.put("data", 3);
							resultMap.put("message", "令牌与设备不匹配");
							logger.error("validateToken:令牌与设备不匹配");
						}
					}
				}else {
					if(java.util.Objects.equals(String.valueOf(resultMap.get("data")), "1")) {
		        		...
		        		resultMap1.put("data", new JSONObject());
		        		logger.error("validateToken:jwt过期，认证失败,");
			        	responseMessage(response, response.getWriter(), resultMap1);
					}else {
						Map<String, Object> resultMap1 = Maps.newHashMap();
		        		resultMap1.put("code", 401);
		        		...
		        		resultMap1.put("desc", "jwt 认证失败");
		        		resultMap1.put("data", new JSONObject());
		        		logger.error("validateToken:jwt 认证失败");
			        	responseMessage(response, response.getWriter(), resultMap1);
					}
				}
			}else {
...
//				if(StringUtils.isBlank(appId)) {
					resultMap.put("message", "无令牌");
//				}
				logger.error("validateToken:无令牌");
				responseMessage(response, response.getWriter(), resultMap);
			}
		} catch (IOException e) {
			e.printStackTrace();
